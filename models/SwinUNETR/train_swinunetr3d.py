...
class Config:
    ...
    # sliding window (val/test)
    sw_batch_size: int = 1
    sw_overlap: float = 0.25
    eval_use_sliding_window: bool = True
...
def build_transforms(cfg: Config):
    ...
    aug = [
        ...
        transforms.RandAdjustContrastd(keys=["image"], gamma=(0.7, 1.5), prob=0.25),
        transforms.RandShiftIntensityd(keys=["image"], offsets=0.1, prob=0.25),
        transforms.RandScaleIntensityd(keys=["image"], factors=0.1, prob=0.25),
        transforms.RandGaussianNoised(keys=["image"], std=0.05, prob=0.25),
    ]
